...
class UserService {
    ...
    static updateUserProfile = async (userId, updates) => {
        let newAvatarLink;
        if (updates["avatar"]) {
            newAvatarLink = await new ImageModel(updates["avatar"].originalname, updates["avatar"].buffer).upload();
    ...
    static followUser = async (userId, followingUserId) => {
        const result = await new FollowingModel(userId, followingUserId).toggleFollowing();
        return result;
    }
