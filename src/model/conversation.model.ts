...
class ConversationModel {
    ...
    static getConversationByMembers = async (memberIds: string[]) => {
        ...
        const result = await database.collection("conversation").find({
            "members.userId": {
                "$all": memberIds
            }, 
            members: { 
    ...
    static getAllConversationsByUserId = async (userId) => {
        const database = getDB();
        const result = await database.collection("conversation").aggregate([
        ...
        const displayedConversations = result.filter(conversation => conversation.messages.length);
        return displayedConversations;
    }

    static getMessagesInConversation = async (conversationId) => {
        const database = getDB();
        const result = await database.collection("message").find({
            conversationId
        }).toArray();
        return result;
    }
